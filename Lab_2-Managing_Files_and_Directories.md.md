...
11. Open `report1.txt` in a text editor (e.g., `nano` or `vim`) and add some sample content:
    ...
    ```
11. Add content to `log1.txt` describing a log entry:
    ```plaintext
    Log Entry: Class started at 09:00 AM CT.
    ```

### Task 4: Move Files
12. Move `summary.txt` to the `configs` directory using relative paths:
    ```bash
    mv -v workspace/reports/summary.txt workspace/configs/
    ```

### Task 5: Copy Files
13. Copy `overview.txt` to the `logs` directory using absolute paths:
    ```bash
    cp -v /home/student/workspace/reports/overview.txt /home/student/workspace/logs/
    ```

### Task 6: Rename Files
14. Rename `config1.txt` to `settings.txt` while working in the *~/workspace/configs/* directory:
    ```bash
...
15. List all files in the `reports` directory:
    ```bash
    ls ../reports
    ```
16. List all files in the `logs` directory:
    ```bash
    ls ../logs
    ```

### Task 8: Remove Files and Directories
17. Delete the `settings.txt` file:
    ```bash
    rm -v settings.txt
    ```
18. Remove the `logs` directory and its contents:
    ```bash
    rm -rv ../logs
    ```

### Task 9: Check Disk Usage
19. Check the disk usage of the `workspace` directory:
    ```bash
    du -sh ~/workspace
    ```

### Task 10: Find Files
20. Find all `.txt` files in the `workspace` directory:
    ```bash
